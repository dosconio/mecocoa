// ASCII C/C++ TAB4 CRLF
// Docutitle: Video Player Application (playvdo)
...
static void PrintUsage(const char* prog_name) {
	outsfmt("Mecocoa Video Player (playvdo)\n\r\n\r");
	outsfmt("Usage: %s [options] <video_file.avi>\n\r\n\r", prog_name ? prog_name : "playvdo");
...
int main(int argc, char** argv)
{
	...
	if (!video.Open(file_path, loop_mode)) {
		outsfmt("playvdo: failed to open or parse video '%s'\n\r", file_path);
		return -1;
	}

	VideoInfo vinfo{};
	if (!video.GetInfo(vinfo) || vinfo.videoFormat.width == 0 || vinfo.videoFormat.height == 0) {
		outsfmt("playvdo: invalid video format or dimensions\n\r");
		video.Close();
		return -1;
	}
	...
	outsfmt("playvdo: '%s' %ux%u @ %u fps, duration: %02u:%02u, total frames: %u%s\n\r",
	...
	if (!canvas) {
		outsfmt("playvdo: out of memory allocating canvas\n\r");
		video.Close();
		return -1;
	}
	...
	if (form_id < 0) {
		outsfmt("playvdo: failed to create form window (code %d)\n\r", form_id);
		free(canvas);
		video.Close();
		return -1;
	}
